    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        ...
        if self.dec_layers > 0:
            ...
            if self.two_stage:
                ...
                ref_ts_learn = refpoint_embed[..., :ts_len, :]    # (bs, ts_len, 4)
                ref_ts_base  = refpoint_embed_ts                  # (bs, ts_len, 4)

                if self.bbox_reparam:
                    cxcy = ref_ts_learn[..., :2] * ref_ts_base[..., 2:] + ref_ts_base[..., :2]
                    wh   = ref_ts_learn[..., 2:].exp() * ref_ts_base[..., 2:]
                    new_ts = torch.cat([cxcy, wh], dim=-1)
                else:
                    new_ts = ref_ts_learn + ref_ts_base

                ref_rest = refpoint_embed[..., ts_len:, :]        # (bs, nq-ts_len, 4)
                refpoint_embed = torch.cat([new_ts, ref_rest], dim=-2)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.refpoint_head = MLP(2 * d_model, d_model, d_model, 2)
    ...
    def refpoints_refine(self, refpoints_unsigmoid, new_refpoints_delta):
        if self.bbox_reparam:
            ref_box = refpoints_unsigmoid[..., :4]
            delta_box = new_refpoints_delta[..., :4]

            new_refpoints_cxcy = delta_box[..., :2] * ref_box[..., 2:] + ref_box[..., :2]
            new_refpoints_wh = delta_box[..., 2:].exp() * ref_box[..., 2:]

            if refpoints_unsigmoid.size(-1) > 4:
                ref_extra = refpoints_unsigmoid[..., 4:]
                delta_extra = new_refpoints_delta[..., 4:]
                new_refpoints_extra = ref_extra + delta_extra
                new_refpoints_unsigmoid = torch.concat(
                    [new_refpoints_cxcy, new_refpoints_wh, new_refpoints_extra], dim=-1
                )
            else:
                new_refpoints_unsigmoid = torch.concat(
    ...
    def forward(self, tgt, memory,
        ...
        def get_reference(refpoints):
            ...
            query_pos = self.refpoint_head(query_sine_embed)
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not self.lite_refpoint_refine:
                if self.refpoint_embed is not None:
                    new_refpoints_delta = self.refpoint_embed(output)
                else:
